...
void BH1750_Start()
{
    GPIO_SetBits(GPIOA, sda);                    //拉高数据线
    GPIO_SetBits(GPIOA, scl);                   //拉高时钟线
    Delay_5us();                 //延时
    GPIO_ResetBits(GPIOA, sda);                    //产生下降沿
    Delay_5us();                 //延时
    GPIO_ResetBits(GPIOA, scl);                    //拉低时钟线
}

/*****停止信号******/
void BH1750_Stop()
{
    GPIO_ResetBits(GPIOA, sda);                   //拉低数据线
    GPIO_SetBits(GPIOA, scl);                      //拉高时钟线
    Delay_5us();                 //延时
    GPIO_SetBits(GPIOA, sda);                    //产生上升沿
    Delay_5us();                 //延时
}
...
uchar BH1750_RecvByte()
{
   ...
   GPIO_Init( GPIOA, &GPIO_InitStruct );
    ...
    GPIO_SetBits( GPIOA, sda);          //使能内部上拉,准备读取数据,
    for (i=0; i<8; i++)         //8位计数器
    {
        dat <<= 1;
        GPIO_SetBits( GPIOA, scl);               //拉高时钟线
