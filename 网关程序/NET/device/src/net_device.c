...
void NET_DEVICE_IO_Init(void)
{
	...
	gpioInitStruct.GPIO_Pin = NET_RST_Pin;					//GPIOA0-复位
	gpioInitStruct.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(NET_RST_Port, &gpioInitStruct);
...
{
  char NET_Buf[20];
	char LCD_SHOW_SSID[20];
	char LCD_SHOW_KEY[20];
	...
	while(timeOut--)												//等待
	{
		...
		if(NET_IO_WaitRecive() == REV_OK)							//如果收到数据
		{
			...
			else if(strstr((char *)netIOInfo.buf, "WIFI DISCONNECT"))
			{
				...
				sprintf(NET_Buf,(const char *)"AT+CWJAP=\"%s\",\"%s\"\r\n",SSID1,wifiKEY1);
				sprintf(LCD_SHOW_SSID,(const char *)"%s",SSID1);
				sprintf(LCD_SHOW_KEY,(const char *)"%s",wifiKEY1);
				POINT_COLOR=RED;
				while(NET_DEVICE_SendCmd(NET_Buf, "GOT IP", 1))
				{}
